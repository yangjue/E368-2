...
static void LCDDrawSensorG1Slope();
static void LCDDrawSensorG2Slope();
static void LCDDrawSensorG1Offset();
static void LCDDrawSensorG2Offset();
static void LCDDrawError();
static void LCDReDrawError();
static void LCDLoadSetting();
static void LCDReDrawLoadSetting();
...
static void lcdcmd_setting ( int  );
...
static void lcdcmd_SetSlope1(int up_down);
static void lcdcmd_SetSlope2(int up_down);
static void lcdcmd_SetOffset1(int up_down);
static void lcdcmd_SetOffset2(int up_down);
static void lcdcmd_nextlog(int up_down);
static void lcdcmd_loadsetting(int up_down);
	//{ LCDDrawSensorG2Slope,lcdcmd_SetS2slope,LCDReDrawSensorG2Slope}, 
///Callback funtion that redraws the home screen
static void LCDReDrawHome( void );
static void LCDReDrawSetting( void ); 
///Callback funtion that redraws the Group Select screen
static void LCDReDrawGroupSelect( void ); 
///Callback function that redraws the Trans page
static void lcdcmd_Trans(int up_down);
...
static void LCDReDrawSensorG1Slope();
static void LCDReDrawSensorG2Slope();
static void LCDReDrawSensorG1Offset();
static void LCDReDrawSensorG2Offset();
static void LCDReDrawTrans();
#define SAFEON 1
#define SAFEOFF 0
...
static short isSysdefault = 1;
static short booted = 1;
...
//#define LCDMENU_N  42
//#define LCDMENU_N  43  //Add Error page
#define LCDMENU_N  44  //Add boot setting page
int MenuJmp = 0;
int TransJmp = 0;
int Setting_LorS = 2;
int Setting_grp = 0;
int Setting_dir = 0;
int Setting_opt = 1; // define which is in control , Load/Save or Group 0: save or load , 1:group, 2:next 
static SLCDMenu lcdMenu[LCDMENU_N] = {
    { LCDDrawHome, 	lcdcmd_home,          LCDReDrawHome},
    { LCDDrawSetting,   lcdcmd_setting,   LCDReDrawSetting },
	...
	{ LCDDrawFlowMeter1Optional,lcdcmd_FMOpt1,LCDReDrawFlowMeter1Optional},  //-- 19 
	{ LCDDrawSensorG1Slope,lcdcmd_SetSlope1,LCDReDrawSensorG1Slope}, 
	{ LCDDrawSensorG1Offset,lcdcmd_SetOffset1,LCDReDrawSensorG1Offset}, 
	{ LCDDrawTrans,lcdcmd_Trans,LCDReDrawTrans}, 
	...
	{ LCDDrawSensorG2Slope,lcdcmd_SetSlope2,LCDReDrawSensorG2Slope}, 
	{ LCDDrawSensorG2Offset,lcdcmd_SetOffset2,LCDReDrawSensorG2Offset}, 
	{ LCDDrawError,lcdcmd_nextlog,LCDReDrawError}, 
	{ LCDLoadSetting,lcdcmd_loadsetting,LCDReDrawLoadSetting},
};

///Current screen
//static int lcdMenu_i = 0;
static int lcdMenu_i = 43;  // add setting page
...
void buttonIterate( void ) {
	int tmp_bs_l = 0,tmp_bs_r = 0;
    if ( risingEdge( BUTTON_L, &bs_l, NULL ) ) {
		if ( risingEdge( BUTTON_R, &tmp_bs_l, NULL ) ) {
			lcdMenu_i = 0;
			LCDFIX = 0;
			return ;
		}
        if (lcdMenu_i == 0 && booted ==0 ){
            lcdMenu_i = LCDMENU_N - 2;
			Setting_LorS = 2;
		}
        else if (lcdMenu_i == 22 && (TransJmp == 0 || TransJmp == 2) ){  // group trans page

		} else if( lcdMenu_i == 1 && Setting_opt == 2){ 
			if (Setting_LorS == 2 ){
				lcdMenu_i--;
				lcdMenu_i = abs(lcdMenu_i);
				lcdMenu_i %= LCDMENU_N;
		}else{
				Setting_dir--;
				Setting_dir = abs(Setting_dir);
			}
			LCDDrawHome();
		}
		else if(lcdMenu_i == 1 && Setting_opt != 2){
			if (Setting_opt == 0){
				Setting_LorS--;
				Setting_LorS = abs(Setting_LorS);
				Setting_LorS = Setting_LorS%3;
			}
			if (Setting_opt == 1){
				Setting_grp--;
				Setting_grp = abs(Setting_grp);
				Setting_grp = Setting_grp%2;
			}
			LCDReDrawSetting(); 
		}
		else{
    ...
    if ( risingEdge( BUTTON_R, &bs_r, NULL ) ) {
		if ( risingEdge( BUTTON_L, &tmp_bs_r, NULL ) ) {
			lcdMenu_i = 0;
			LCDFIX = 0;
			return ;
		}
		...
		if (lcdMenu_i == 2) {
			if (MenuJmp != 0){
				lcdMenu_i = lcdMenu_i + 21; }// depend on trans page	
			else{
				lcdMenu_i++;
			}
		}else if(lcdMenu_i == 22 && TransJmp == 0) { // group trans page
			lcdMenu_i++;
		}
		else if( lcdMenu_i == 22 && TransJmp == 1) {
		}
		else if( lcdMenu_i == 22 && TransJmp == 2) {
			lcdMenu_i = 0;
		}
		else if( lcdMenu_i == 1 && Setting_opt == 2){ 
			if ( Setting_LorS != 2 ){  
				switch (Setting_LorS) {
					...
					case 1:
						if (Setting_grp == 0 )
						  savesetting(S_GROUP1);
						else 
						  savesetting(S_GROUP2);
						break;
				}
				lcd_print( 1, 0, "jump in 5s ...    ", 20);
				Setting_LorS = 2;
			}else {
			}
			if (Setting_dir == 1) {
...
static void LCDDrawSetting(){
    lcd_print( 0, 0, "Group Setting       ", 20);
    lcd_print( 1, 0, "Load/Save/SKIP    + ", 20);
    lcd_print( 2, 0, "GROUP #1 OR #2   < >", 20);
    lcd_print( 3, 0, "Do Then FORW/BACK - ", 20);
}
...
static void LCDDrawTrans(){
    lcd_print( 0, 0, "Group Trans         ", 20);
    lcd_print( 1, 0, "Forward   <--       ", 20);
    lcd_print( 2, 0, "Backward         < >", 20);
    lcd_print( 3, 0, "Home              - ", 20);
}
//------------------------------------------------------------------------------
...
void LCDDrawModeFullString(group *g) {
    ...
    case MODE_RAMP_QC:
        ...
        break;
    case MODE_PWM_QC:
        lcd_print( 2, 0, "PWM_QC          ", 17 );
        lcd_print( 3, 0, "Heater/ Fan Ctr  ", 17 );
        break;
    default:
...
void LCDDrawSensorG1Slope(){

	lcd_print( 0, 0, "Set Sensor Slope    ", 20);
    lcd_print( 1, 0, "GROUP #1          +",  20);
    lcd_print( 2, 0, "                 < >", 20);
    lcd_print( 3, 0, "                  - ", 20);
}
void LCDDrawSensorG2Slope(){
...
void LCDDrawSensorG1Offset(){

	lcd_print( 0, 0, "Set Sensor Offset   ", 20);
    lcd_print( 1, 0, "GROUP #1          +",  20);
    lcd_print( 2, 0, "                 < >", 20);
    lcd_print( 3, 0, "                  - ", 20);
}
void LCDDrawSensorG2Offset(){
...
void LCDLoadSetting(){
	lcd_print( 0, 0, "Select Setting      ", 20);
    lcd_print( 1, 0, "sys default       + ", 20);
    lcd_print( 2, 0, "                 < >", 20);
    lcd_print( 3, 0, "                  - ", 20);
}
//------------------------------------------------------------------------------
...
static void lcdcmd_setting( int i){
	if (i > 0)
		Setting_opt--;
	else 
		Setting_opt++;
	Setting_opt = Setting_opt%3;
	LCDReDrawSetting();
}
static void lcdcmd_Trans( int i){
	TransJmp = TransJmp - i;
	if (TransJmp < 0)
	  TransJmp = 2;
	TransJmp = (TransJmp)%3;
...
void lcdcmd_settempstep1( int up_down ) {
	if ( up_down == -1 || up_down == 1 ) {
	  ...
	  set_group_temp_step( S_GROUP1, get_group_temp_step(S_GROUP1) + 50 * up_down);
	}
...
void lcdcmd_settempstep2( int up_down ) {
	if ( up_down == -1 || up_down == 1 ) {
	  set_group_temp_step( S_GROUP1, get_group_temp_step(S_GROUP2) + 25 * up_down);
	}
	else { 
	  set_group_temp_step( S_GROUP1, get_group_temp_step(S_GROUP2) + 50 * up_down);
	}
}
//------------------------------------------------------------------------------
...
void lcdcmd_SetSlope1( int up_down) {
	float f = 0.0;
	group *g = S_GROUP1;
	f = g->slope + 0.1*up_down;
	setslope(S_GROUP1,f);
}
void lcdcmd_SetSlope2( int up_down) {
	float f = 0.0;
	group *g = S_GROUP2;
	f = g->slope + 0.1*up_down;
	setslope(S_GROUP2,f);
}
void lcdcmd_SetOffset1( int up_down) {
	float f = 0.0;
	group *g = S_GROUP1;
	f = g->offset + 0.1*up_down;
	setoffset(S_GROUP1,f);
}
void lcdcmd_SetOffset2( int up_down) {
    //set_group_slope(S_GROUP2, get_group_offset(S_GROUP2) + 0.1 * up_down);
	float f = 0.0;
	group *g = S_GROUP2;
	f = g->offset + 0.1*up_down;
	setoffset(S_GROUP2,f);
}

void lcdcmd_nextlog(int up_down) {
	return ;
}
void lcdcmd_loadsetting(int up_down) {
	if (isSysdefault) 
		isSysdefault = 0;
	else 
	    isSysdefault = 1;
...
void LCDReDrawSetting() {
  ...
  switch(Setting_dir) {
	  case 0:
		  strcpy(direction,"Forw   prs'>'   - ");
		  break;
	  case 1:
		  strcpy(direction,"Back   prs'>'   - ");
		  break;
  }
  switch(Setting_opt) {
	  case 0:
  ...
  lcd_print( 1, 0, LorS, 20);
  lcd_print( 2, 0, grp, 20);
  lcd_print( 3, 0, direction, 20);
}
...
void LCDReDrawSetMode1() {
    ...
    switch ( i ) {
    case MODE_AUTO:
        lcd_print( 1, 9, "AUTO   ", 7 );
        break;
    case MODE_PELT:
        lcd_print( 1, 9, "PELT   ", 7 );
        break;
    case MODE_IDLE:
        lcd_print( 1, 9, "IDLE   ", 7 );
        break;
    case MODE_FANON:
        lcd_print( 1, 9, "FANON  ", 7 );
        break;
    case MODE_RAMP:
    	lcd_print( 1, 9, "RAMP   ", 7 );
        break;
    case MODE_RAMP_QC:
        lcd_print( 1, 9, "RAMPQ  ", 7 );
        break;
    case MODE_PWM_QC:
        lcd_print( 1, 9, "PWM_QC ", 7 );
        break;
    default:
        lcd_print( 1, 9, "N/A    ", 7 );
    }
...
void LCDReDrawSensorG1Slope(){
	char buf[8];
	group *g = S_GROUP1;
	formatfloatstr(g->slope,buf);
	buf[5] = '\0';

    lcd_print( 2, 9, buf, 5 );
}
void LCDReDrawSensorG2Slope(){
	char buf[8];
	group *g = S_GROUP2;
	formatfloatstr(g->slope,buf);
	buf[5] = '\0';
...
void LCDReDrawSensorG1Offset(){
	char buf[8];
	group *g = S_GROUP1;
	formatfloatstr(g->offset,buf);
	buf[5] = '\0';
    lcd_print( 2, 9, buf, 5 );
}
void LCDReDrawSensorG2Offset(){
	char buf[8];
	group *g = S_GROUP2;
	formatfloatstr(g->offset,buf);
	buf[5] = '\0';
...
void LCDReDrawError() {
    //char bug[2][8];
	short l = strlen(errorLog1[lasterror1-1]) ;
    lcd_print( 1, 4, errorLog1[lasterror1-1], l );
	l = strlen(errorLog2[lasterror2-1]) ;
    lcd_print( 2, 4, errorLog2[lasterror2-1], l );
}
void LCDReDrawLoadSetting() {
    //lcd_print( 0, 0, "Select Setting      ", 20);
	if ( isSysdefault ) 
        lcd_print( 1, 0, "sys default       + ", 20);
	else
        lcd_print( 1, 0, "from ROM          + ", 20);
